...
template sudoku() {
    ...
    for (var i=0; i<9; i++) {
        for (var j=0; j<9; j++) {
            mul.out[i][j] === 0;
            // Assign outs for range proof components
            rp_puzzle[i][j].out === 1;
            rp_solution[i][j].out === 1;
        }
    }
